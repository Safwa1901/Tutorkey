...
class _PendingTuteeState extends State<PendingTutee> {
  ...
  Widget build(BuildContext context) {
    ...
    // Use `where()` to filter pending sessions
    final sessionData = session.where((element) => element.status == 'pending').toList();
              ...
              return StreamBuilder<Profile>(
                ...
                builder: (BuildContext context, AsyncSnapshot<Profile> snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    return Loading(); // Display loading while waiting for data
